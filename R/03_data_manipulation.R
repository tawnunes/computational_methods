...
for (i in 1:length(files_path)) {
  data <- read.csv(files_path[[i]])# reading the csv in the files_path in i position
  assign(file_names[i], data) # is the same as assining "<-" someting to the name (first variable)
}
