...
```{r plot-create}
# Defining the theme
n_breaks <- hist(comm_df$n, plot = F)$breaks

p <- ggplot(comm_df) +
  geom_point(mapping = aes(x = sp, y = n), size = 3, alpha = 0.5) +
  labs(x = "Species rank", y = "Abundance") +
  scale_y_continuous(breaks = n_breaks) +
  annotate("text", x = 16, y = 126, 
           label = "italic(Bolboschoenus~maritimus)", 
           parse = TRUE, size = 2) +
  annotate("text", x = 16, y = 80, 
           label = "italic(Phalaris~coerulescenss)", 
           parse = TRUE, size = 2) +
  theme_classic()
```
...
Inside of the header of e chunk {r,} we can add a name for the chunk and a description of the figures as an alternative for people that can't se using _fi.alt ='text'_. If you click on the figure the alternative text is going to appear. For this you need to really describe the image in a way that text could substitute it.
